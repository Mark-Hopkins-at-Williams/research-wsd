...
def lemmadata_iter(lemmadir):
    ...
    """
    with open(join(lemmadir, "id_to_sent.json")) as sent_id_dict_file:
        sent_id_dict = json.load(sent_id_dict_file)
    for dir_item in os.listdir(lemmadir):
        filename = join(lemmadir, dir_item)
...
def specified_sense_historgrams(lemmas, lemmadir):
    """
...
def contextualized_vectors_by_sense(lemma, vectorize, lemmadir, use_cached = False):
    """
    ...
    def contextualized_vectors(lemma):    
        for instance in lemmadata(lemma, lemmadir):
...
def sample_sense_pairs(vectorize, n_pairs, lemma, lemmadir, 
    ...
    vecs_by_sense = contextualized_vectors_by_sense(lemma, vectorize, 
                                                    lemmadir, use_cached=cached)
...
def sample_inputids_pairs(n_pairs, lemma, sense1, sense2, n_fold, train_percent = 0.8):
    ...
    def sample_negative_sense_pair(sense_vecs):
        ...
        vec1 = random.choice(sense_vecs[sense1])
        vec2 = random.choice(sense_vecs[sense2])
        return torch.cat([vec1[:1], vec2[:1], vec1[1:], vec2[1:]]).detach() #different from sample_sense_pairs
    ...
    def sample(sense1_vecs, sense2_vecs):
        ...
        negatives = torch.stack([sample_negative_sense_pair(vecs) for
                                 _ in range(n_pairs)])
        positives = torch.cat([torch.ones(len(positives), 1, dtype=torch.long), positives], #different from sample_sense_pairs
                               dim=1)
        negatives = torch.cat([torch.zeros(len(negatives), 1, dtype=torch.long), negatives], #different from sample_sense_pairs
                               dim=1)
        return torch.cat([negatives, positives]).detach()

    idized_vecs_by_sense = tokens_to_ids_by_sense(lemma) #different from sample_sense_pairs
    sense1_vecs = idized_vecs_by_sense[sense1] #different from sample_sense_pairs
    sense2_vecs = idized_vecs_by_sense[sense2] #different from sample_sense_pairs
    print('sampling {} with senses of magnitude {} and {}'.format(lemma, 
          len(sense1_vecs), len(sense2_vecs)))
