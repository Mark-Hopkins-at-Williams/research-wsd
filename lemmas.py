...
from bert import embed_bert
from elmo import embed_elmo
...
def contextualized_vectors_by_sense(lemma, model):
    """
...
def contextualized_vectors_by_sense_elmo(lemma):
    def contextualized_vectors(lemma, batch_size=48):
        count = 0
        vecs = []
        positions = []
        senses = []
        for instance in lemmadata(lemma, "elmo"):
            if len(instance.tokens) > 511:
                continue
            if count >= batch_size:
                count = 0
                embeddings = embed_elmo(positions, vecs)
...
def sample_sense_pairs(model, n_pairs, lemma, sense1, sense2, n_fold, cached=True, train_percent = 0.8):
    """
...
def sample_cross_lemma(model, threshold, n_fold, n_pairs_each_lemma):
    """
    ...
    for (lemma, sense_hist) in all_sense_histograms(model):
        if len(sense_hist) > 1 and sense_hist[1][0] >= 21 and lemma in train_lemmas:
            ...
            train_data = sample_sense_pairs(model, n_pairs_each_lemma//2, lemma, sense1, sense2, n_fold)
    ...
    for (lemma, sense_hist) in all_sense_histograms(model):
        if len(sense_hist) > 1 and sense_hist[1][0] >= 21 and lemma in test_lemmas:
            sense1 = sense_hist[0][1]
            sense2 = sense_hist[1][1]
            test_data = sample_sense_pairs(model, n_pairs_each_lemma//2, lemma, sense1, sense2, n_fold, cached=True)
...
def tokens_to_ids_by_sense_bert(lemma):
...
def sample_inputids_pairs_bert(n_pairs, lemma, sense1, sense2, n_fold, train_percent = 0.8):
    ...
    idized_vecs_by_sense = tokens_to_ids_by_sense_bert(lemma)
