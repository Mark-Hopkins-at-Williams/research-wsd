...
class TestBEMDataset(unittest.TestCase):
    ...
    def test_BEMDataset_subset(self):
        #test semcor subset feature
        ds = wordsense.BEMDataset(self.sents, sense_sz=2)
        assert(len(ds) == 2)
        
        ds = wordsense.BEMDataset(self.sents, sense_sz=1)
        assert(len(ds) == 1)

        ds = wordsense.BEMDataset(self.sents)
        inv_d = {'be': ['be%2:42:06::'],
                 'screen': ['screen%1:06:06::']}
        inv = wordsense.SenseInventory(inv_d)
        ds.set_inventory(inv)
        assert(len(ds) == 2)

    def test_wn_example(self):
        ds = wordsense.BEMDataset(self.sents, randomize_sents = False)
        word = 'is'
        be = wn.lemma_from_key('be%2:42:03::')
        gloss, span = ds.wn_example(be, word)
        assert(gloss == 'John is rich')
        assert(span == [2, 3])

        word = 'cat'
        cat = wn.lemma_from_key('cat%1:05:00::')
        gloss, span = ds.wn_example(cat, word)
        assert(gloss == word + ' is ' + 'feline mammal usually having thick soft fur and no ability to roar: domestic cats; wildcats')
        assert(span == [1, 2])
    ...
    def test_item(self):
        ...
        #defn_tgt
        ds = wordsense.BEMDataset(self.sents, randomize_sents=False, gloss='defn_tgt')
        inst1 = ds[1]
        ...
        #wneg
        ds = wordsense.BEMDataset(self.sents, randomize_sents=False, gloss='wneg')
        inst2 = ds[2]
        expected_context = torch.tensor([101, 2002, 2001, 4191, 1035, 2125, 1996, 3898, 1012, 102])
